...
from .models import Categoria, Usuario, Perfil, Departamento, Atividade
...
def rel_departamentos(request):
    ...
    return render(request, 'rel_departamentos.html', {'departamentos_lista': departamentos_lista})
#----------------------------------------------------
...
def categorias(request):
    ...
    if request.method == "POST":
        ...
        if acao == "nova_categoria":
            ...
            categoria_id = request.POST.get('txtId')
            categoria = Categoria.objects.get(id=categoria_id)

            categoria.nome = request.POST.get('txtNome')
            categoria.save()
    ...
    paginator = Paginator(categorias_lista, settings.NUMBER_GRID_PAGES)
    numero_pagina = request.GET.get('page')
    page_obj = paginator.get_page(numero_pagina)

    return render(request, "categorias.html", {'page_obj': page_obj})

@login_required
def obter_categoria_por_id(request):
    categoria_id = request.GET.get('categoria_id', None)
    categoria = Categoria.objects.get(id=categoria_id)
    
    #responsaveis
    responsaveis = categoria.responsaveis.values('id', 'nome').order_by('nome')
    ...
    return JsonResponse(categoria_dados)
...
@login_required
def pesquisar_categoria_por_nome(request):
    categoria_nome = request.GET.get('categoria_nome', '')
    numero_pagina = request.GET.get('page')
    
    categorias_lista = Categoria.objects.filter(nome__icontains=categoria_nome).order_by('nome')

    paginator = Paginator(categorias_lista, settings.NUMBER_GRID_PAGES)
    page_obj = paginator.get_page(numero_pagina)

    return JsonResponse({
        'html': render_to_string('categorias_table.html', {'page_obj': page_obj, 'query': categoria_nome, 'request': request})
    })

#RELACIONANDO categorias COM RESPONSAVEIS (USUARIOS)
@login_required
def adicionar_responsavel_de_categoria(request):
    categoria_id = request.POST.get('categoria_id')
    responsavel_id = request.POST.get('responsavel_id')

    categoria = Categoria.objects.get(id=categoria_id)
    responsavel = Usuario.objects.get(id=responsavel_id)
    categoria.responsaveis.add(responsavel)
    
    return JsonResponse({'success': True, 'message': 'Responsavel adicionado com sucesso!'})
    
@login_required
def excluir_responsavel_de_categoria(request):
    categoria_id = request.POST.get('categoria_id')
    responsavel_id = request.POST.get('responsavel_id')

    categoria = Categoria.objects.get(id=categoria_id)
    responsavel = Usuario.objects.get(id=responsavel_id)
    categoria.responsaveis.remove(responsavel)
    
    return JsonResponse({'success': True, 'message': 'Responsavel removido com sucesso!'})

@require_POST    
@login_required
def exibir_responsaveis_possiveis_para_categoria(request):
    categoria_id = request.POST.get('txtPostIdResponsaveis')    
    responsaveis = Usuario.objects.order_by('nome').exclude(nome__iexact="Administrador")

    responsaveis_ja_associados = []
    if categoria_id:
        categoria = Categoria.objects.get(id=categoria_id)
        responsaveis_ja_associados = list(categoria.responsaveis.values_list('id', flat=True))
    ...
    #carrega a pagina de adicao de responsaveis a categorias
    return render(request, 'categorias_add_responsaveis.html', context)

@login_required
def pesquisar_responsavel_por_nome_para_categoria(request):
    nome = request.GET.get('nome', '')
    page_number = request.GET.get('page', 1)
    ...
    #carrega o grid com responsaveis atualizado dentro de categorias_add_responsaveis.html
    html = render_to_string('categorias_add_responsaveis_table.html', context)
    return JsonResponse({'html': html})